...
use core_v3::{
    benchmarks::{
        bench_v3_decrypt_reader::{
            bench_v3_decrypt_reader_2_file_sync,
            bench_v3_decrypt_reader_2_memory_sync,
            bench_v3_decrypt_reader_2_writer_sync
        },
        bench_v3_encrypt_reader::{
            bench_v3_encrypt_reader_2_file_sync,
            bench_v3_encrypt_reader_2_memory_sync,
            bench_v3_encrypt_reader_2_writer_sync
        }
    }
};
...
pub fn run_sync_benchmarks(_master_key: &MasterKey) -> Vec<BenchmarkResult> {
    ...
    let (enc_result_mem, ciphertext_mem, _) = bench_v3_encrypt_reader_2_memory_sync(
    ...
    let (dec_result_mem, plaintext_mem, _) = bench_v3_decrypt_reader_2_memory_sync(
    ...
    let (enc_res_writer, ciphertext_writer, _) = bench_v3_encrypt_reader_2_writer_sync(
        input_reader.clone(), chunk_size, compression, parallelism.clone()
    );
    ...
    let (dec_result_writer, plaintext_writer, _) = bench_v3_decrypt_reader_2_writer_sync(
        output_reader.clone(), chunk_size, compression, parallelism.clone()
    );
    ...
    // Encrypt Reader → File
    let (enc_res_file, _, enc_file_path) = bench_v3_encrypt_reader_2_file_sync(
        input_reader.clone(), chunk_size, compression, parallelism.clone()
    );
    ...
    // Decrypt Reader → File
    let (dec_result_file, _, dec_file_path) = bench_v3_decrypt_reader_2_file_sync(
        output_reader.clone(), chunk_size, compression, parallelism.clone()
    );
...
// cargo run -p core-api --release --example bench_v3_reader --features benchmarks
