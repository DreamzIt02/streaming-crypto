...
use crate::v3::stream::core::{ApiConfig, EncryptParams, encrypt_stream_v3};
...
macro_rules! bench_v3_encrypt_reader {
    ($out_variant:ident, $out_expr:expr) => {
        paste::paste! {
            pub fn [<bench_v3_encrypt_reader_2_ $out_variant:lower _sync>](
...
bench_v3_encrypt_reader!(Memory, OutputSink::Memory);

// Encrypt from Reader → File
bench_v3_encrypt_reader!(File, OutputSink::File(PathBuf::from("encrypted_reader.dat")));

// Encrypt from Reader → Writer
bench_v3_encrypt_reader!(Writer, OutputSink::Writer(Box::new(Vec::new())));
...
//     // Encrypt from Reader → Memory
//     let (res_mem, ciphertext_mem, _) = bench_v3_encrypt_reader_2_memory_sync(
...
//     // Encrypt from Reader → File
//     let (res_file, _, file_path) = bench_v3_encrypt_reader_2_file_sync(
//         payload_size, chunk_size, compression, parallelism
//     );
...
//     // Encrypt from Reader → Writer
//     let (res_writer, ciphertext_writer, _) = bench_v3_encrypt_reader_2_writer_sync(
//         payload_size, chunk_size, compression, parallelism
//     );
