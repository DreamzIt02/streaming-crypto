...
macro_rules! bench_v3_decrypt_reader {
    ($out_variant:ident, $out_expr:expr) => {
        paste::paste! {
            pub fn [<bench_v3_decrypt_reader_2_ $out_variant:lower _sync>](
...
bench_v3_decrypt_reader!(Memory, OutputSink::Memory);

// Decrypt from Reader → File
bench_v3_decrypt_reader!(File, OutputSink::File(PathBuf::from("decrypted_reader.dat")));

// Decrypt from Reader → Writer
bench_v3_decrypt_reader!(Writer, OutputSink::Writer(Box::new(Vec::new())));
...
//     let (enc_result, ciphertext, _) = bench_v3_encrypt_reader_2_memory_sync(
...
//     // Decrypt from Reader → File
//     let (dec_result_file, _, file_path) = bench_v3_decrypt_reader_2_file_sync(
//         ciphertext.clone().unwrap(),
//         chunk_size, compression, parallelism
//     );
