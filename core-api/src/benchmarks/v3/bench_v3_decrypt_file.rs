...
macro_rules! bench_v3_decrypt_file {
    ($out_variant:ident, $out_expr:expr) => {
        paste::paste! {
            pub fn [<bench_v3_decrypt_file_2_ $out_variant:lower _sync>](
...
bench_v3_decrypt_file!(Memory, OutputSink::Memory);

// Decrypt from File → File
bench_v3_decrypt_file!(File, OutputSink::File(PathBuf::from("decrypted_file.dat")));

// Decrypt from File → Writer
bench_v3_decrypt_file!(Writer, OutputSink::Writer(Box::new(Vec::new())));

// ### 📂 Expanded Functions
// This generates three functions:

// - `bench_v3_decrypt_file_2_memory_sync`
// - `bench_v3_decrypt_file_2_file_sync`
// - `bench_v3_decrypt_file_2_writer_sync`
...
//     let (enc_result, _, file_path) = bench_v3_encrypt_file_2_file_sync(
//         chunk_size, compression, parallelism
//     );
//     println!("Encrypt File→File result: {:?}", enc_result);

//     // Decrypt from that file → Memory
//     let (dec_result, plaintext, _) = bench_v3_decrypt_file_2_memory_sync(
