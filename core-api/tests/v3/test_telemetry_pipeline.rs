...
mod telemetry_pipeline_tests {
    ...
    use core_api::{
        ...
        }, stream_v3::pipeline::{decrypt_pipeline, decrypt_read_header, encrypt_pipeline}, telemetry::TelemetrySnapshot
    };
    ...
    fn run_encrypt_with_data(data: &[u8]) -> (TelemetrySnapshot, Vec<u8>) {
        ...
        let snapshot = encrypt_pipeline(InputSource::Memory(&data), &mut writer, Arc::new(ctx), &config_pipe, log_manager).unwrap();
        println!("{}", snapshot);
        (snapshot, writer.into_inner())
    }

    fn run_decrypt_with_ciphertext(ciphertext: Vec<u8>) -> TelemetrySnapshot {
        let cursor = InputSource::Memory(&ciphertext);
        let (stream_header, reader) = decrypt_read_header(cursor).unwrap();
        let mut writer = Cursor::new(Vec::new());
        let (ctx, profile, log_manager) = setup_dec_context(DigestAlg::Blake3, &stream_header);
        let config_pipe = PipelineConfig::new(profile.clone(), None);
        decrypt_pipeline(reader, &mut writer, Arc::new(ctx), &config_pipe, log_manager).unwrap()
    }

    fn run_decrypt_with_data(data: &[u8]) -> TelemetrySnapshot {
        let (snapshot, ciphertext) = run_encrypt_with_data(data);
        let snapshot_dec = run_decrypt_with_ciphertext(ciphertext);
        print!("{}", snapshot);
        print!("");
        print!("{}", snapshot_dec);
        snapshot_dec
    ...
    fn telemetry_fails_on_corrupted_ciphertext() {
        ...
        let reader = InputSource::Memory(&bad_ciphertext);
        let mut writer = Cursor::new(Vec::new());
        let (ctx, profile, log_manager) = setup_dec_context(DigestAlg::Blake3, &HeaderV1::test_header());
        let config_pipe = PipelineConfig::new(profile.clone(), None);
        let result = decrypt_pipeline(reader, &mut writer, Arc::new(ctx), &config_pipe, log_manager);
        assert!(result.is_err());
    }
