...
use core_api::{
    benchmarks::{bench_results::BenchmarkResult, bench_runner::{ApiVersion, BenchFuture, bench_main}, bench_utils::{dummy_master_key, random_bytes}, bench_v3_decrypt_file::{bench_v3_decrypt_file_2_file_sync, bench_v3_decrypt_file_2_memory_sync, bench_v3_decrypt_file_2_writer_sync}, bench_v3_encrypt_file::{bench_v3_encrypt_file_2_file_sync, bench_v3_encrypt_file_2_memory_sync, bench_v3_encrypt_file_2_writer_sync}}, 
...
pub fn run_sync_benchmarks(_master_key: &MasterKey) -> Vec<BenchmarkResult> {
    ...
    // Encrypt File → File
    let (enc_res_file, _, enc_file_path) = bench_v3_encrypt_file_2_file_sync(
    ...
    // Decrypt File → File
    let (dec_result_file, _, dec_file_path) = bench_v3_decrypt_file_2_file_sync(
    ...
    // Encrypt File → Memory
    let (enc_result_mem, ciphertext_mem, _) = bench_v3_encrypt_file_2_memory_sync(
        input_file.clone(), chunk_size, compression, parallelism.clone()
    );
    ...
    // Decrypt File → Memory
    let (dec_result_mem, plaintext_mem, _) = bench_v3_decrypt_file_2_memory_sync(
        ciphertext_path.clone(), chunk_size, compression, parallelism.clone()
    );
    ...
    // Encrypt File → Writer
    let (enc_res_writer, ciphertext_writer, _) = bench_v3_encrypt_file_2_writer_sync(
        input_file.clone(), chunk_size, compression, parallelism.clone()
    );
    ...
    // Decrypt File → Writer
    let (dec_result_writer, plaintext_writer, _) = bench_v3_decrypt_file_2_writer_sync(
...
// cargo run -p core-api --release --example bench_v3_file --features benchmarks

// ### How It Works
// - `bench_v3_file.rs` defines its own `main`.
// - When we run this file as a binary (`cargo run -p crypto-core --bin bench_v3_file`), it executes our local `main`.
